...
+(instancetype)timeCellWithTableView:(UITableView *)tableview {
    static NSString *celldntify = @"shijia";
    AttendanceTableViewCell *cell = [tableview dequeueReusableCellWithIdentifier:celldntify];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"AttendanceTableViewCell" owner:nil options:nil]lastObject];
    }
    return cell;
}

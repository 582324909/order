...
@interface ModifyAddressTableViewController ()
...
- (void)viewDidLoad {
    ...
    if (self.nameBlock&&self.sexBlock&&self.telephoneBlock&&self.shopNameBlock&&self.addressBlock) {
        self.name.text = self.nameBlock();
        self.telephone.text = self.telephoneBlock();
        self.address.text = self.addressBlock();
        self.shopName.text = self.shopNameBlock();
        if ([self.sexBlock() isEqualToString:@"先生"]) {
...
-(void)modifyShopInfo {
    ...
    [httpSession POST:modifyShopInfoURL parameters:parameter success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
